...
class SqlInsertBuilder {
    ...
    static def main(args) {
        def outputFile = new File("D:\\IProjects\\tourGambling\\grails-app\\conf\\sql\\countryTeamRiderInserts.sql")
...
"""
        ...
        new File("D:\\IProjects\\tourGambling\\grails-app\\conf\\sql\\rawRiderList.txt").text.eachLine { line ->
            ...
            if (!rider[0].isInteger()) {
                ...
                outputFile << "insert into team (version, name, country_id) values (0, '${teamName.trim()}',(select id from country where abbreviation = '${country}'));\n "
                ...
                if (rider.size() == 4) {
                    ...
                    outputFile << "insert into rider (bib_number, version, first_name, last_name, team_id, nationality_id) values (${rider[0]}, 0,'${rider[1]}','${rider[2]}',(select id from team where name = '${teamName.trim()}'),(select id from country where abbreviation = UPPER('${country}')));\n"
                } else {
                }
            }
        }
        println "\r\n-->SQL file written.  Hack away."
    }
